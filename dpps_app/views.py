...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Registration successful! Fill form below to login.')
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            return redirect('dashboard')  # Hapa ni sehemu ambapo unataka mtumiaji aende baada ya ku-login.
        else:
            # Hapa tunawonyesha error message
            messages.error(request, "Invalid username or password")
            return render(request, 'login.html')  # Re-render the login page
    return render(request, 'login.html')  # Initial rendering
